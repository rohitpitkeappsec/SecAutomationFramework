...
app.get('/css/:file',function(req,res){  
   res.sendFile('css/'+req.params.file, { root: path.join(__dirname, '')});
});

app.get('/js/:file',function(req,res){  
  res.sendFile('js/'+req.params.file, { root: path.join(__dirname, '')});
});
      ...
      if(jsonBody.status=='Success')
      {
        ...
        getRandomNumber(64, function(status, randomNumber){
          if(status === false){
            ...
            res.cookie('csrfToken', randomNumber, {httpOnly : true});
            res.redirect('/home/');
         }
        });
      } else {
...
app.get('/home/', function(req, res){
  if (req.session.userAuth == true){
	...
	res.send("Some error occured");
        ...
        res.render("index.jade" ,{dataclient : clientID, csrf: req.session.csrfCookie});
      }
    });
  } else { 
    res.redirect('/');
...
app.get('/gettools/:clientid', function(req,res){
  if(req.session.userAuth == true){
	...
	res.send("Some error occured");
        ...
        res.render("toollist.jade" ,{tools : toolList, client: req.params.clientid, csrf: req.session.csrfCookie});
      }
    });
  } else {
    res.redirect('/');
...
app.get('/gettoolinfo/:clientid/:toolid', function(req,res){
  if(req.session.userAuth == true){
	...
	res.send("Some error occured on client");
        ...
        res.render("toolinfo.jade" ,{keys : keys, values: jsonBody, csrf: req.session.csrfCookie});
      }
    });
  } else {
    res.redirect('/');
  ...
  if(req.session.userAuth == true){
    ...
    if(csrfBodyToken == csrfCookie){
      ...
      for(var i=0; i<keys.length; i++){
        var values = {"value": req.body[keys[i]]}
	...
	if (error){
	  ...
	  res.send("Some error occured");
          ...
          var jsonBody = JSON.parse(body);
          res.render("runtool.jade", {scanid: jsonBody.scanID, csrf: req.session.csrfCookie});
	}
      });
    } else {
      res.send("Invalid CSRF token");
...
app.get('/report', function(req, res){
  if(req.session.userAuth == true){
    res.render("report.jade", {csrf: req.session.csrfCookie});
  } else {
    res.redirect('/');
  }
});
...
app.get('/tools', function(req, res){
  if(req.session.userAuth == true){
    res.render("tools.jade", {csrf: req.session.csrfCookie});
  } else {
    res.redirect('/');
  }
});
...
app.get('/uploadtoserver', function(req, res){
  if(req.session.userAuth == true){
    res.render("uploadserver.jade", {csrf: req.session.csrfCookie});
  } else {
    res.redirect('/');
  }
});
...
app.get('/pushtoclient', function(req, res){
  if(req.session.userAuth == true){
	...
	res.send("Some error occured");
        ...
        res.render("toolselect.jade" ,{clientid : req.session.userClients, toollist: toolList, csrf: req.session.csrfCookie});
      }
    });
  } else {
    res.redirect('/');
...
app.post('/tooltoclient', function(req, res){
  if(req.session.userAuth == true){
    ...
    if(csrfBodyToken == csrfCookie){
      ...
      var body = {"clientID":clientid, "toolID":toolid};
      var headers = {
        'Content-Type':'application/json'
      }
      var options = {
	...
	if (error){
	  ...
	  res.send("Some error occured");
          ...
          var toolList = JSON.parse(body).toolList;
          res.render("pushstatus.jade" ,{csrf: req.session.csrfCookie});
	}
      });
    } else {
      res.send("Invalid CSRF token");
...
app.get('/settings', function(req, res){
  if(req.session.userAuth == true){
    res.render("settings.jade", {csrf: req.session.csrfCookie});
  } else {
    res.redirect('/');
  }
});
...
app.get('/adduserui', function(req, res){
  if(req.session.userAuth == true){
    res.render("adduserui.jade", {csrf: req.session.csrfCookie});
  } else {
    res.redirect('/');
  }
});
...
app.post('/adduser', function(req, res){
  if(req.session.userAuth == true){
    ...
    if(csrfBodyToken == csrfCookie){
      var body = {"username":req.body.userid, "password":req.body.password,"email":req.body.email};
      var headers = {
        'Content-Type':'application/json'
      }
      var options = {
	...
	if (error){
	  ...
	  res.send("Some error occured");
          ...
          res.render("adduserstatus.jade", {csrf: req.session.csrfCookie});
	}
      });
    } else {
      res.send("Invalid CSRF token");
  ...
  if(req.session.userAuth == true){
	...
	request(userOpt, function(error, userRes, userBody){
	  if (error){
	    ...
	    res.render("userclientui.jade", {clientid : clientList, userid: userList, csrf: req.session.csrfCookie});
	  }
	});
      }
    });
...
app.post('/userclientmap', function(req, res){
  if(req.session.userAuth == true){
    ...
    if(csrfBodyToken == csrfCookie){
      ...
      //console.log(clients);
      var body = {"username":req.body.userid, "clientID":clients};
      var headers = {
        'Content-Type':'application/json'
      }
      var options = {
	...
	if (error){
	  ...
	  res.send("Some error occured");
          ...
          res.render("mapuserclientstatus.jade", {csrf: req.session.csrfCookie});
	}
      });
    } else {
      res.send("Invalid CSRF token");
