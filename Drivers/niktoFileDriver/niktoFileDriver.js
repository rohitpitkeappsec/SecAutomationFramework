...
var file = 'niktoFileDriver.json';    //configuration file placed in the same directory
...
var runTool = function (scanID, userJSONData, callback){
  fs.readFile(configPath + file, 'utf8', function (err, configJSONData){    //function for reading config file content 
    ...
    if(userJSONData.URL.value.indexOf(str) > -1)    //Checking for malicious data such as ';'
      ...
      var data = [];
      var command = "echo n | nikto -Tuning 1570 -h ";   // Interesting file, remote file in server and web root , file upload test
      ...
      cp.exec(command, function (err, stdout, stderr) {
        ...
        data.push(JSONData);
        var datasend = {"scanID":scanID,"toolNPM":"niktoFileDriver",  "date":currDate,"data":data}
        callback(datasend);
