...
var file = 'nmapCipherDriver.json';    //configuration file placed in the same directory
...
var getToolInfo = function (callback) {
  fs.readFile(configPath + file, 'utf8', function (err, Data)  {  //function for reading config file content 
    ...
    delete JSONData.port.commandOption;
    callback(JSONData);  
  });
}
...
var runTool = function (scanID, userJSONData, callback){
  fs.readFile(configPath + file, 'utf8', function (err, configJSONData){    //function for reading config file content 
    ...
    if(userJSONData.IP.value.indexOf(str) > -1 || 
      userJSONData.port.value.indexOf(str) > -1 )    //Checking for malicious data such as ';'
      callback("User input is Malicious");  
    else{    
      var JSONinput ="", JSONoutput="", message = "";
      var data = [];
      var command = "nmap --script ssl-enum-ciphers -p ";
      command = command + userJSONData.port.value;
      ...
      cp.exec(command, function (err, stdout, stderr) {
        ...
        data.push(JSONData);
        var datasend = {"scanID":scanID,"toolNPM":"nmapCipherDriver", "date":currDate, "data":data}
        callback(datasend);
