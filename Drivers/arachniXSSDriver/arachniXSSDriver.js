...
var file = 'arachniXSSDriver.json';    //configuration file placed in the same directory
...
var runTool = function (scanID, userJSONData, callback){
  fs.readFile(configPath + file, 'utf8', function (err, configJSONData){    //function for reading config file content 
    ...
    if(userJSONData.URL.value.indexOf(str) > -1)    //Checking for malicious data such as ';'
      ...
      var data = [];
      var command = "arachni --output-only-positives --scope-exclude-binaries --scope-page-limit 50 --checks=xss_path ";
      ...
      cp.exec(command, function (err, stdout, stderr) {
        ...
        JSONoutput = stdout;
        message = "Arachni XPATH Scan Result";
        var currDate = Date();
        var JSONData = {"input":JSONinput, "output":JSONoutput, "message":message};
        data.push(JSONData);
        var datasend = {"scanID":scanID,"toolNPM":"arachniXSSDriver",  "date":currDate,"data":data}
        callback(datasend);
